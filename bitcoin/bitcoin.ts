...
export function prepareTx({
  ...
  amount,
  address,
}: BuildTxOptions): PrepareTxResult {
      ...
      if (v >= 0) {
        if (v >= DUST_AMOUNT) {
          ...
          return {
            ok: {
              fee:
                baseInputs.reduce((a, b) => a + b.value, 0) -
                finOutputs.reduce((a, b) => a + b.value, 0),
              inputs: baseInputs,
              feeRate,
              network,
              address,
          ...
          return {
            ok: {
              ...
              fee:
                baseInputs.reduce((a, b) => a + b.value, 0) -
                outputs.reduce((a, b) => a + b.value, 0),
              inputs: baseInputs,
              outputs,
            },
          };
  ...
  if (regularUTXOs.length > 2 && !isP2TR) {
    ...
    for (let i = 0; i < regularUTXOs.length; i++) {
      ...
      if (utxo.value <= perInput) {
        return {
          error:
            "No suitable UTXO is available. Please consider lowering the transaction fee rate.",
        };
      }
      value += utxo.value;
  ...
  signPsbt(psbt: bitcoin.Psbt, opts?: SignOptions) {
    ...
    psbt.data.inputs.forEach((v, index) => {
      ...
      if (script && !isSigned) {
        if (
          this._address ===
          bitcoin.address.fromOutputScript(script, this._network)
        ) {
          toSignInputs.push({
            index,
            pubkey: this._pubkey,
...
export const getTapscriptCommitment = (
...
export function prepareCommitAndRevealTx({
  ...
  revealInputs,
  payload,
}: PrepareCommitAndRevealTxOptions): PrepareCommitAndRevealTxResult {
  ...
  const revealTxFee = calcRevealFee(
    feeRate,
    revealInputs,
    revealOutputs,
...
export function getNetworkType(address: string): NetworkType {
  const mainnetStart = ["1", "3", "bc1q", "bc1p"];
  const testnetStart = ["m", "n", "2", "tb1q", "tb1p"];
  if (mainnetStart.some((v) => address.startsWith(v))) {
    return NetworkType.livenet;
  } else if (testnetStart.some((v) => address.startsWith(v))) {
